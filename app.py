...
def login():
    try:
        ...
        matched_rows = execute_query(userquery, (username,))  # Use a tuple
        ...
        if matched_rows:
            ...
            managerquery = """
                ...
                WHERE Name = %s
            """
            matched_rows = execute_query(managerquery, (username,))
...
def register_employee():
    try:
        ...
        if (typeofaccount=="Employee"):
                ...
                # SQL query to insert a new employee into the user_info table
                insert_employee_query = """
                    INSERT INTO managers_data (Name, ID, Password, TypeOfAccount, Project, MailID, PhoneNo)
                    VALUES (%s, %s, %s, %s, %s, %s, %s)
                """
...
def add_new_project():
    try:
        ...
        query = """
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
        """
        execute_query(query, (project_data['Project'], project_data['Address'], project_data['GSTIN'], 
                             project_data['STATE'], project_data['State_Code'], project_data['Manager']), commit = True)
...
def add_new_manager():
    try:
        ...
        print(f"Name: {manager_name}, ID: {manager_id}, Password: {manager_password}, "
              f"Project: {manager_project}, Email: {manager_email}, Phone: {manager_phone} ///")

        # SQL Query to insert a new manager
        query = """
            INSERT INTO managers_data (Name, ID, Password, Project, MailID, PhoneNo, TypeOfAccount)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """
        toa = "Manager"
        # Execute the query with the extracted data
        execute_query(query, (manager_id, manager_name, manager_id, manager_project, manager_email, manager_phone, toa), commit=True)
